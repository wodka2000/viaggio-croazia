(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const s of o.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function t(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=t(n);fetch(n.href,o)}})();function _e(){const e=document.getElementById("nav-toggle"),a=document.getElementById("nav-links");e==null||e.addEventListener("click",()=>{a==null||a.classList.toggle("open")}),a==null||a.querySelectorAll(".nav-link").forEach(i=>{i.addEventListener("click",()=>{a.classList.remove("open")})});function t(){const i=window.location.hash||"#dashboard";document.querySelectorAll(".nav-link").forEach(n=>{n.classList.toggle("active",n.getAttribute("href")===i)})}window.addEventListener("hashchange",t),t()}let q=null;async function L(){if(q)return q;const t=await fetch("/viaggio-croazia/data/trip.json");if(!t.ok)throw new Error(`Impossibile caricare trip.json (${t.status})`);return q=await t.json(),q}function f(e){return e?new Date(e+"T00:00:00").toLocaleDateString("it-IT",{day:"numeric",month:"short",year:"numeric"}):""}function J(e){if(!e)return"";const[,a,t]=e.split("-");return`${t}/${a}`}function Ie(e){return e?new Date(e+"T00:00:00").toLocaleDateString("it-IT",{weekday:"short"}).toUpperCase():""}function te(e){const a=new Date;a.setHours(0,0,0,0);const t=new Date(e+"T00:00:00");return Math.round((t-a)/(1e3*60*60*24))}function we(e){return"★".repeat(e)+"☆".repeat(5-e)}async function Le(){const e=document.getElementById("page-content");let a;try{a=await L()}catch(u){e.innerHTML=Se(u.message);return}const{meta:t,days:i,hotels:n}=a,o=te(t.start_date),s=te(t.end_date),c=new Date().toISOString().slice(0,10),l=i.find(u=>u.date===c),d=i.find(u=>u.date>c),r=l||d,m=[],p=new Set;for(const u of i)p.has(u.location)||(p.add(u.location),m.push({location:u.location,day:u.day,date:u.date}));const b=n.filter(u=>u.recommended),M=b.reduce((u,k)=>u+k.nights,0);b.filter(u=>u.price_per_night>0).reduce((u,k)=>u+k.nights*k.price_per_night,0),e.innerHTML=`
  ...
  `).join("")}function De(e){const a=document.querySelector(".timeline");a&&(a.addEventListener("click",t=>{var s,c;const i=t.target.closest(".day-add-idea-btn");if(i){const l=i.dataset.date,d=document.getElementById(`quick-form-${l}`);if(!d)return;const r=d.classList.contains("hidden");document.querySelectorAll(".day-quick-form").forEach(m=>m.classList.add("hidden")),r&&(d.classList.remove("hidden"),(s=d.querySelector(".quick-idea-text"))==null||s.focus())}const n=t.target.closest(".quick-cancel");n&&((c=n.closest(".day-quick-form"))==null||c.classList.add("hidden"));const o=t.target.closest(".day-idea-del");if(o){const l=o.dataset.id;confirm("Eliminare questa idea?")&&ge(l)}}),a.addEventListener("submit",t=>{var d,r,m,p;const i=t.target.closest(".day-quick-form");if(!i)return;t.preventDefault();const n=(d=i.querySelector(".quick-idea-text"))==null?void 0:d.value.trim();if(!n){(r=i.querySelector(".quick-idea-text"))==null||r.focus();return}const o=i.dataset.date,s=e.find(b=>b.date===o),c=(m=i.querySelector(".quick-map"))==null?void 0:m.checked,l=(p=i.querySelector(".quick-checklist"))==null?void 0:p.checked;Z({text:n,day_date:o,location_name:(s==null?void 0:s.location)??null,coordinates:c&&(s!=null&&s.coordinates)?s.coordinates:null,add_to_checklist:!!l,add_to_map:!!(c&&(s!=null&&s.coordinates))}),i.querySelector(".quick-idea-text").value="",i.querySelector(".quick-map").checked=!1,i.querySelector(".quick-checklist").checked=!1,i.classList.add("hidden")}))}function Pe(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}async function Ne(){const e=document.getElementById("page-content");let a;try{a=await L()}catch(l){e.innerHTML=`<div class="error-state"><div class="error-icon">⚠️</div><h2>Errore</h2><p>${l.message}</p></div>`;return}const{hotels:t}=a,i={},n=[];for(const l of t){const d=l.location_group||l.location;i[d]||(i[d]=[],n.push(d)),i[d].push(l)}const o=t.filter(l=>l.recommended&&l.price_per_night>0),s=o.reduce((l,d)=>l+d.nights,0),c=o.reduce((l,d)=>l+d.nights*d.price_per_night,0);e.innerHTML=`
    <div class="page-header">
      <h1>🏨 Hotel &amp; Alloggi</h1>
    ...
    `}).join("")}
  `}function He(e){const a=e.nights,t=a*e.price_per_night,n=e.booking_ref&&e.booking_ref.length>0?'<span class="hotel-status-badge badge-prenotato">✅ Prenotato</span>':e.status==="da_confermare"?'<span class="hotel-status-badge badge-confermare">⚠️ Da confermare</span>':'<span class="hotel-status-badge badge-da-prenotare">📋 Da prenotare</span>',o=e.recommended?'<span class="hotel-recommended-badge">⭐ Consigliato</span>':'<span class="hotel-alt-badge">Alternativa</span>';return`
    <div class="hotel-card ${e.recommended?"hotel-card--recommended":""}">
      ...
      <div class="hotel-card-body">
        ...
        <div class="hotel-links">
          ...
          <a href="https://www.google.com/search?q=${encodeURIComponent(e.name+" "+e.address)}"
             target="_blank" rel="noopener" class="hotel-link-btn">
            🔍 Cerca online
          </a>
        </div>
  ...
  `,S(t,i),Xe(t,i),ie(),G();const n=()=>{const s=document.getElementById("checklist-ideas-section");s&&(s.innerHTML=ae(),ie())},o=()=>{const s=document.getElementById("custom-items-section");s&&(s.innerHTML=V(),G())};window.addEventListener("ideas:updated",n),window.addEventListener("customitems:updated",o),window.__currentPageCleanup=()=>{window.removeEventListener("ideas:updated",n),window.removeEventListener("customitems:updated",o)}}function ae(){const e=Ae();if(e.length===0)return"";const a=e.filter(n=>n.completed).length,t=e.length,i=t?a/t*100:0;return`
  ...
  `}function G(){var a,t,i;const e=document.getElementById("custom-items-header");e==null||e.addEventListener("click",n=>{var o,s;n.target.closest(".custom-add-form")||n.target.closest("button")||((o=document.getElementById("custom-items-list"))==null||o.classList.toggle("hidden"),(s=document.getElementById("custom-chev"))==null||s.classList.toggle("open"))}),(a=document.getElementById("custom-items-list"))==null||a.addEventListener("change",n=>{const o=n.target;o.classList.contains("custom-item-cb")&&Ge(o.dataset.cid,o.checked)}),(t=document.getElementById("custom-items-list"))==null||t.addEventListener("click",n=>{const o=n.target.closest(".custom-item-del");if(o){We(o.dataset.cid);return}const s=n.target.closest(".custom-item-edit");if(s){Ze(s.dataset.cid);return}if(n.target.closest("#custom-add-btn")){ne();return}n.target.closest("#clear-custom-btn")&&confirm("Eliminare tutte le voci personalizzate?")&&Je()}),(i=document.getElementById("custom-item-text"))==null||i.addEventListener("keydown",n=>{n.key==="Enter"&&(n.preventDefault(),ne())})}function ne(){var i;const e=document.getElementById("custom-item-text"),a=((i=document.getElementById("custom-item-cat"))==null?void 0:i.value)??"Varie",t=e==null?void 0:e.value.trim();if(!t){e==null||e.focus();return}Ve(t,a),e&&(e.value="")}function Ze(e){const a=x().find(p=>p.id===e);if(!a)return;const t=document.querySelector(`.custom-item[data-cid="${e}"]`);if(!t)return;const i=t.querySelector(".checklist-item-text"),n=t.querySelector(".custom-item-edit"),o=t.querySelector(".custom-item-del"),s=t.querySelector(".priority-badge");if(!i)return;const c=document.createElement("input");c.type="text",c.value=a.text,c.className="custom-edit-input";const l=document.createElement("button");l.textContent="✓",l.className="custom-item-save-edit",l.title="Salva";const d=document.createElement("button");d.textContent="✕",d.className="custom-item-cancel-edit",d.title="Annulla",i.replaceWith(c),s&&(s.style.display="none"),n&&n.replaceWith(l),o&&(o.style.display="none"),l.after(d),c.focus(),c.select();function r(){const p=c.value.trim();if(!p){c.focus();return}Ue(e,p)}function m(){const p=document.getElementById("custom-items-section");p&&(p.innerHTML=V(),G())}c.addEventListener("keydown",p=>{p.key==="Enter"&&(p.preventDefault(),r()),p.key==="Escape"&&m()}),l.addEventListener("click",r),d.addEventListener("click",m)}function Ye(e,a){const t=e.items.length,i=e.items.filter(n=>a[n.id]).length;return`
    <div class="category-card" data-cat="${e.id}">
      ...
      <div class="category-items" id="items-${e.id}">
        ${e.items.map(n=>Qe(n,a[n.id]||!1)).join("")}
      </div>
    </div>
  ...
  `}function Xe(e,a){var t,i,n,o;(t=document.getElementById("checklist-categories"))==null||t.addEventListener("change",s=>{var d;const c=s.target;if(c.type!=="checkbox")return;const l=c.dataset.id;a[l]=c.checked,(d=c.closest(".checklist-item"))==null||d.classList.toggle("checked",c.checked),R(a),et(e,a,l),S(e,a)}),document.querySelectorAll(".category-header").forEach(s=>{s.addEventListener("click",c=>{var d,r,m;if(c.target.tagName==="INPUT")return;const l=(d=s.closest(".category-card"))==null?void 0:d.dataset.cat;l&&((r=document.getElementById(`items-${l}`))==null||r.classList.toggle("hidden"),(m=document.getElementById(`chev-${l}`))==null||m.classList.toggle("open"))})}),(i=document.getElementById("check-all"))==null||i.addEventListener("click",()=>{e.forEach(s=>s.items.forEach(c=>{a[c.id]=!0})),R(a),j(e,a),S(e,a)}),(n=document.getElementById("uncheck-all"))==null||n.addEventListener("click",()=>{e.forEach(s=>s.items.forEach(c=>{a[c.id]=!1})),R(a),j(e,a),S(e,a)}),(o=document.getElementById("reset-btn"))==null||o.addEventListener("click",()=>{confirm("Vuoi resettare l'intera checklist?")&&(je(),e.forEach(s=>s.items.forEach(c=>{a[c.id]=!1})),j(e,a),S(e,a))})}function et(e,a,t){const i=e.find(d=>d.items.some(r=>r.id===t));if(!i)return;const n=i.items.length,o=i.items.filter(d=>a[d.id]).length,s=n?o/n*100:0,c=document.getElementById(`count-${i.id}`),l=document.getElementById(`prog-${i.id}`);c&&(c.textContent=`${o}/${n}`),l&&(l.style.width=`${s}%`)}function S(e,a){const t=e.reduce((c,l)=>c+l.items.length,0),i=e.reduce((c,l)=>c+l.items.filter(d=>a[d.id]).length,0),n=t?i/t*100:0,o=document.getElementById("global-fill"),s=document.getElementById("global-text");o&&(o.style.width=`${n}%`),s&&(s.textContent=`${i} / ${t}`)}function j(e,a){e.forEach(t=>{const i=t.items.length,n=t.items.filter(l=>a[l.id]).length,o=i?n/i*100:0,s=document.getElementById(`count-${t.id}`),c=document.getElementById(`prog-${t.id}`);s&&(s.textContent=`${n}/${i}`),c&&(c.style.width=`${o}%`),t.items.forEach(l=>{const d=document.querySelector(`.checklist-item[data-id="${l.id}"]`),r=d==null?void 0:d.querySelector('input[type="checkbox"]');!d||!r||(r.checked=a[l.id]||!1,d.classList.toggle("checked",r.checked))})})}function O(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const tt="AIzaSyCAFbyemD6aGU_oemr3qi_CFc6UHpaMsU8";let v=null,U=[],W=[],z=[],T=null,oe=null;async function at(){const e=document.getElementById("page-content");e.innerHTML=`
    <div class="page-header">
  ...
  `,oe=await L(),await it(tt),nt(oe),dt(),se();const a=()=>{if(!v)return;z.forEach(i=>i.setMap(null)),z=[],be(),se();const t=document.querySelector('.map-btn[data-layer="ideas"]');if(t){const i=P().length;t.textContent=`💡 Idee${i>0?` (${i})`:""}`}};window.addEventListener("ideas:updated",a),window.__currentPageCleanup=()=>window.removeEventListener("ideas:updated",a)}function it(e){return new Promise((a,t)=>{var o;if((o=window.google)!=null&&o.maps){a();return}const i="__gmapsReady_"+Date.now();window[i]=()=>{delete window[i],a()};const n=document.createElement("script");n.src=`https://maps.googleapis.com/maps/api/js?key=${e}&callback=${i}&loading=async`,n.async=!0,n.onerror=()=>t(new Error("Impossibile caricare Google Maps")),document.head.appendChild(n)})}function nt(e){const a=document.getElementById("google-map");a&&(v=new google.maps.Map(a,{center:{lat:44,lng:15.5},zoom:7,mapTypeId:"roadmap",styles:[{featureType:"water",elementType:"geometry",stylers:[{color:"#a2daf5"}]},{featureType:"landscape",stylers:[{color:"#f5f5f0"}]}]}),ot(e),st(e),lt(e),be(),ct(e))}function ot(e){W=[];const a=new Map;e.days.filter(t=>t.coordinates).forEach(t=>{const i=`${t.coordinates.lat},${t.coordinates.lng}`;a.has(i)||a.set(i,[]),a.get(i).push(t)}),a.forEach(t=>{const i=t[0],n=t[t.length-1],o=t.length>1?`Giorni ${i.day}–${n.day} — ${i.location}`:`Giorno ${i.day} — ${i.location}`,s=new google.maps.Marker({position:i.coordinates,map:v,title:i.location,label:{text:String(i.day),color:"#fff",fontWeight:"bold",fontSize:"11px"},icon:{path:google.maps.SymbolPath.CIRCLE,scale:16,fillColor:"#1e40af",fillOpacity:1,strokeColor:"#ffffff",strokeWeight:2},zIndex:10}),c=t.map(d=>`<div style="margin-top:5px;font-size:12px;border-top:1px solid #e2e8f0;padding-top:4px;">
        ...
        <div style="font-family:system-ui,sans-serif;max-width:240px;">
          <strong style="color:#1e40af;">${o}</strong>
          ${c}
  ...
  `,H(),ke(),ye(),gt();const t=()=>{$(),w()};window.addEventListener("ideas:updated",t),window.__currentPageCleanup=()=>window.removeEventListener("ideas:updated",t)}function N(e){var s,c;const a=!!e,t=A.map(l=>`<option value="${l.date}" ${(e==null?void 0:e.day_date)===l.date?"selected":""}>
    ...
    </option>`).join(""),i=F.map(l=>`<option value="${l.value}" ${((e==null?void 0:e.stato)??"idea")===l.value?"selected":""}>${l.label}</option>`).join(""),n=me.map(l=>`<option value="${l.value}" ${((e==null?void 0:e.categoria)??"varia")===l.value?"selected":""}>${l.label}</option>`).join(""),o=pe.map(l=>`<option value="${l.value}" ${((e==null?void 0:e.priorita)??"media")===l.value?"selected":""}>${l.label}</option>`).join("");return`
  ...
  `}function H(){var a,t,i;const e=document.getElementById("ideas-form-wrap");e&&((a=document.getElementById("ideas-toggle-advanced"))==null||a.addEventListener("click",()=>{h=!h;const n=document.getElementById("ideas-advanced"),o=document.getElementById("ideas-toggle-advanced");n==null||n.classList.toggle("hidden",!h),o&&(o.textContent=h?"▲ Meno dettagli":"▼ Più dettagli")}),e.addEventListener("change",n=>{var o;if(n.target.id==="idea-day"){const s=A.find(c=>c.date===n.target.value);s!=null&&s.coordinates&&(document.getElementById("idea-lat").value=s.coordinates.lat,document.getElementById("idea-lng").value=s.coordinates.lng,document.getElementById("idea-coords-hint").textContent=`Auto-riempito da "${s.location}"`)}n.target.id==="idea-add-map"&&((o=document.getElementById("idea-color-row"))==null||o.classList.toggle("hidden",!n.target.checked))}),(t=document.getElementById("idea-form"))==null||t.addEventListener("submit",n=>{n.preventDefault(),ut()}),(i=document.getElementById("idea-cancel-btn"))==null||i.addEventListener("click",()=>{_=null,document.getElementById("ideas-form-wrap").innerHTML=N(null),H()}))}function ut(){var s,c,l,d,r,m,p,b,M,u,k,Q,X,ee;const e=(s=document.getElementById("idea-text"))==null?void 0:s.value.trim();if(!e){(c=document.getElementById("idea-text"))==null||c.focus();return}const a=parseFloat((l=document.getElementById("idea-lat"))==null?void 0:l.value),t=parseFloat((d=document.getElementById("idea-lng"))==null?void 0:d.value),i=!isNaN(a)&&!isNaN(t)?{lat:a,lng:t}:null,n=(r=document.getElementById("idea-add-map"))==null?void 0:r.checked,o={text:e,note:((m=document.getElementById("idea-note"))==null?void 0:m.value.trim())??"",categoria:((p=document.getElementById("idea-categoria"))==null?void 0:p.value)??"varia",stato:((b=document.getElementById("idea-stato"))==null?void 0:b.value)??"idea",priorita:((M=document.getElementById("idea-priorita"))==null?void 0:M.value)??"media",day_date:((u=document.getElementById("idea-day"))==null?void 0:u.value)||null,location_name:((k=document.getElementById("idea-location"))==null?void 0:k.value.trim())||null,link:((Q=document.getElementById("idea-link"))==null?void 0:Q.value.trim())||"",coordinates:i,add_to_checklist:((X=document.getElementById("idea-add-checklist"))==null?void 0:X.checked)??!1,add_to_map:n&&i!==null,marker_color:((ee=document.querySelector('input[name="idea-color"]:checked'))==null?void 0:ee.value)??"#f59e0b"};_?(B(_,o),_=null):Z(o),h=!1,document.getElementById("ideas-form-wrap").innerHTML=N(null),H(),$(),w()}function he(){const e=g();return[{key:"tutte",label:`Tutte (${e.length})`},{key:"idea",label:`Idea (${e.filter(t=>t.stato==="idea").length})`},{key:"da-verificare",label:`Da verificare (${e.filter(t=>t.stato==="da-verificare").length})`},{key:"prenotare",label:`Prenotare (${e.filter(t=>t.stato==="prenotare").length})`},{key:"approvata",label:`Approvate (${e.filter(t=>t.stato==="approvata").length})`},{key:"scartata",label:`Scartate (${e.filter(t=>t.stato==="scartata").length})`}].map(t=>`
    <button class="idea-filter-btn ${I===t.key?"active":""}" data-f="${t.key}">
      ${t.label}
    </button>
  `).join("")}function ye(){var e;(e=document.getElementById("ideas-filters-row"))==null||e.querySelectorAll(".idea-filter-btn").forEach(a=>{a.addEventListener("click",()=>{I=a.dataset.f,w(),$()})})}function w(){const e=document.getElementById("ideas-filters-row");e&&(e.innerHTML=he(),ye())}function $e(){let e=g();return I!=="tutte"&&(e=e.filter(a=>a.stato===I)),e.length===0?`<div class="ideas-empty">${I==="tutte"?"Nessuna idea salvata. Aggiungine una qui sopra.":"Nessuna idea per questo filtro."}</div>`:e.map(mt).join("")}function mt(e){const a=F.find(o=>o.value===e.stato),t=me.find(o=>o.value===e.categoria),i=pe.find(o=>o.value===e.priorita),n=e.day_date?A.find(o=>o.date===e.day_date):null;return`
    <div class="idea-card ${e.stato==="scartata"?"idea-card--scartata":""}" data-id="${e.id}">
      ...
      <div class="idea-card-actions">
        ...
        <select class="idea-stato-select" data-action="stato" data-id="${e.id}" title="Cambia stato">
          ${F.map(o=>`<option value="${o.value}" ${e.stato===o.value?"selected":""}>${o.label}</option>`).join("")}
        </select>
  ...
  `}function pt(e){var n,o,s;const a=g().find(c=>c.id===e);if(!a)return;(n=document.getElementById("day-link-modal"))==null||n.remove();const t=A.map(c=>`<option value="${c.date}" ${a.day_date===c.date?"selected":""}>
