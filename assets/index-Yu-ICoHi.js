(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const s of o.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function t(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=t(n);fetch(n.href,o)}})();function ge(){const e=document.getElementById("nav-toggle"),a=document.getElementById("nav-links");e==null||e.addEventListener("click",()=>{a==null||a.classList.toggle("open")}),a==null||a.querySelectorAll(".nav-link").forEach(i=>{i.addEventListener("click",()=>{a.classList.remove("open")})});function t(){const i=window.location.hash||"#dashboard";document.querySelectorAll(".nav-link").forEach(n=>{n.classList.toggle("active",n.getAttribute("href")===i)})}window.addEventListener("hashchange",t),t()}let C=null;async function L(){if(C)return C;const t=await fetch("/viaggio-croazia/data/trip.json");if(!t.ok)throw new Error(`Impossibile caricare trip.json (${t.status})`);return C=await t.json(),C}function v(e){return e?new Date(e+"T00:00:00").toLocaleDateString("it-IT",{day:"numeric",month:"short",year:"numeric"}):""}function ve(e){return e?new Date(e+"T00:00:00").toLocaleDateString("it-IT",{weekday:"short"}).toUpperCase():""}function K(e){const a=new Date;a.setHours(0,0,0,0);const t=new Date(e+"T00:00:00");return Math.round((t-a)/(1e3*60*60*24))}function be(e){return"★".repeat(e)+"☆".repeat(5-e)}async function fe(){const e=document.getElementById("page-content");let a;try{a=await L()}catch(u){e.innerHTML=ye(u.message);return}const{meta:t,days:i,hotels:n}=a,o=K(t.start_date),s=K(t.end_date),c=new Date().toISOString().slice(0,10),l=i.find(u=>u.date===c),d=i.find(u=>u.date>c),r=l||d,m=[],p=new Set;for(const u of i)p.has(u.location)||(p.add(u.location),m.push({location:u.location,day:u.day,date:u.date}));const b=n.filter(u=>u.recommended),A=b.reduce((u,$)=>u+$.nights,0);b.filter(u=>u.price_per_night>0).reduce((u,$)=>u+$.nights*$.price_per_night,0),e.innerHTML=`
    <div class="dashboard-hero">
      <h1>${t.emoji} ${t.title}</h1>
      <p class="subtitle">${t.subtitle}</p>
      <div class="dates">
        <span class="date-badge">✈️ ${v(t.start_date)}</span>
        <span style="color:rgba(255,255,255,0.5);">→</span>
        <span class="date-badge">${v(t.end_date)}</span>
  ...
  `}const ie="viaggio_croazia_ideas_v1",ne=[{value:"alloggio",label:"🏨 Alloggio"},{value:"ristorante",label:"🍽️ Ristorante"},{value:"esperienza",label:"🎯 Esperienza"},{value:"spiaggia",label:"🏖️ Spiaggia"},{value:"escursione",label:"🥾 Escursione"},{value:"cantina",label:"🍷 Cantina / Cibo"},{value:"shopping",label:"🛍️ Shopping"},{value:"varia",label:"💡 Varia"}],P=[{value:"idea",label:"Idea",color:"#6366f1"},{value:"da-verificare",label:"Da verificare",color:"#f59e0b"},{value:"prenotare",label:"Prenotare",color:"#ef4444"},{value:"approvata",label:"Approvata",color:"#10b981"},{value:"scartata",label:"Scartata",color:"#94a3b8"}],oe=[{value:"alta",label:"🔴 Alta"},{value:"media",label:"🟡 Media"},{value:"bassa",label:"🟢 Bassa"}],$e=[{value:"#f59e0b",label:"Giallo"},{value:"#ef4444",label:"Rosso"},{value:"#8b5cf6",label:"Viola"},{value:"#10b981",label:"Verde"},{value:"#f97316",label:"Arancio"}];function ke(){return"idea_"+Date.now().toString(36)+"_"+Math.random().toString(36).slice(2,5)}function R(e){return{categoria:"varia",link:"",priorita:"media",stato:"idea",note:e.note??"",location_name:e.location_name??null,...e}}function g(){try{const e=localStorage.getItem(ie);return e?JSON.parse(e).map(R):[]}catch{return[]}}function M(e){try{localStorage.setItem(ie,JSON.stringify(e)),window.dispatchEvent(new CustomEvent("ideas:updated",{detail:{ideas:e}}))}catch(a){console.warn("[ideas] localStorage non disponibile:",a)}}function F(e){const a=g(),t=R({id:ke(),text:"",note:"",categoria:"varia",link:"",priorita:"media",stato:"idea",created_at:new Date().toISOString(),day_date:null,location_name:null,coordinates:null,add_to_checklist:!1,add_to_map:!1,marker_color:"#f59e0b",completed:!1,...e});return a.unshift(t),M(a),t}function x(e,a){const t=g(),i=t.findIndex(n=>n.id===e);return i===-1?null:(t[i]={...t[i],...a},M(t),t[i])}function se(e){M(g().filter(a=>a.id!==e))}function Ee(e){return g().filter(a=>a.day_date===e)}function _e(){return g().filter(e=>e.add_to_checklist)}function Ie(){return JSON.stringify(g(),null,2)}function Le(e){const a=JSON.parse(e);if(!Array.isArray(a))throw new Error("JSON non valido: deve essere un array");const t=g(),i=new Set(t.map(s=>s.id));let n=0;const o=[...t];return a.forEach(s=>{i.has(s.id)||(o.push(R(s)),n++)}),M(o),n}async function we(){var c,l;const e=document.getElementById("page-content");let a;try{a=await L()}catch(d){e.innerHTML=`<div class="error-state"><div class="error-icon">⚠️</div><h2>Errore</h2><p>${d.message}</p></div>`;return}const{days:t,hotels:i,meta:n}=a,o=Object.fromEntries(i.map(d=>[d.id,d]));e.innerHTML=`
  ...
  `).join("")}function xe(e){const a=document.querySelector(".timeline");a&&(a.addEventListener("click",t=>{var s,c;const i=t.target.closest(".day-add-idea-btn");if(i){const l=i.dataset.date,d=document.getElementById(`quick-form-${l}`);if(!d)return;const r=d.classList.contains("hidden");document.querySelectorAll(".day-quick-form").forEach(m=>m.classList.add("hidden")),r&&(d.classList.remove("hidden"),(s=d.querySelector(".quick-idea-text"))==null||s.focus())}const n=t.target.closest(".quick-cancel");n&&((c=n.closest(".day-quick-form"))==null||c.classList.add("hidden"));const o=t.target.closest(".day-idea-del");if(o){const l=o.dataset.id;confirm("Eliminare questa idea?")&&se(l)}}),a.addEventListener("submit",t=>{var d,r,m,p;const i=t.target.closest(".day-quick-form");if(!i)return;t.preventDefault();const n=(d=i.querySelector(".quick-idea-text"))==null?void 0:d.value.trim();if(!n){(r=i.querySelector(".quick-idea-text"))==null||r.focus();return}const o=i.dataset.date,s=e.find(b=>b.date===o),c=(m=i.querySelector(".quick-map"))==null?void 0:m.checked,l=(p=i.querySelector(".quick-checklist"))==null?void 0:p.checked;F({text:n,day_date:o,location_name:(s==null?void 0:s.location)??null,coordinates:c&&(s!=null&&s.coordinates)?s.coordinates:null,add_to_checklist:!!l,add_to_map:!!(c&&(s!=null&&s.coordinates))}),i.querySelector(".quick-idea-text").value="",i.querySelector(".quick-map").checked=!1,i.querySelector(".quick-checklist").checked=!1,i.classList.add("hidden")}))}function Be(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}async function ze(){const e=document.getElementById("page-content");let a;try{a=await L()}catch(l){e.innerHTML=`<div class="error-state"><div class="error-icon">⚠️</div><h2>Errore</h2><p>${l.message}</p></div>`;return}const{hotels:t}=a,i={},n=[];for(const l of t){const d=l.location_group||l.location;i[d]||(i[d]=[],n.push(d)),i[d].push(l)}const o=t.filter(l=>l.recommended&&l.price_per_night>0),s=o.reduce((l,d)=>l+d.nights,0),c=o.reduce((l,d)=>l+d.nights*d.price_per_night,0);e.innerHTML=`
    <div class="page-header">
      <h1>🏨 Hotel &amp; Alloggi</h1>
    ...
    ${n.map(l=>{var d,r;return`
      <div class="hotels-location-group">
        <div class="hotels-group-header">
          <span class="hotels-group-title">📍 ${l}</span>
          <span class="hotels-group-nights">${(d=i[l][0])!=null&&d.nights?i[l][0].nights+" nott"+(i[l][0].nights===1?"e":"i"):""} · ${(r=i[l][0])!=null&&r.checkin?v(i[l][0].checkin)+" → "+v(i[l][0].checkout):""}</span>
        </div>
    ...
    `}).join("")}
  `}function Ae(e){const a=e.nights,t=a*e.price_per_night,n=e.booking_ref&&e.booking_ref.length>0?'<span class="hotel-status-badge badge-prenotato">✅ Prenotato</span>':e.status==="da_confermare"?'<span class="hotel-status-badge badge-confermare">⚠️ Da confermare</span>':'<span class="hotel-status-badge badge-da-prenotare">📋 Da prenotare</span>',o=e.recommended?'<span class="hotel-recommended-badge">⭐ Consigliato</span>':'<span class="hotel-alt-badge">Alternativa</span>';return`
  ...
  `}const V="viaggio_croazia_checklist_v1",ce="viaggio_croazia_custom_items_v1";function Ce(){try{const e=localStorage.getItem(V);return e?JSON.parse(e):{}}catch{return{}}}function D(e){try{localStorage.setItem(V,JSON.stringify(e))}catch(a){console.warn("[storage] Impossibile salvare checklist:",a)}}function qe(){try{localStorage.removeItem(V)}catch{}}function Me(){return"ci_"+Date.now().toString(36)+"_"+Math.random().toString(36).slice(2,5)}function w(){try{const e=localStorage.getItem(ce);return e?JSON.parse(e):[]}catch{return[]}}function B(e){try{localStorage.setItem(ce,JSON.stringify(e)),window.dispatchEvent(new CustomEvent("customitems:updated"))}catch(a){console.warn("[storage] Impossibile salvare custom items:",a)}}function Te(e,a="Varie"){const t=w(),i={id:Me(),text:e.trim(),category:a,checked:!1,created_at:new Date().toISOString()};return t.unshift(i),B(t),i}function Oe(e,a){B(w().map(t=>t.id===e?{...t,checked:a}:t))}function De(e,a){const t=a.trim();t&&B(w().map(i=>i.id===e?{...i,text:t}:i))}function Ne(e){B(w().filter(a=>a.id!==e))}function Pe(){B([])}async function je(){const e=document.getElementById("page-content");let a;try{a=await L()}catch(s){e.innerHTML=`<div class="error-state"><div class="error-icon">⚠️</div><h2>Errore</h2><p>${s.message}</p></div>`;return}const{categories:t}=a.checklist,i=Ce();e.innerHTML=`
    <div class="page-header">
  ...
  `,S(t,i),Ve(t,i),Y(),H();const n=()=>{const s=document.getElementById("checklist-ideas-section");s&&(s.innerHTML=Z(),Y())},o=()=>{const s=document.getElementById("custom-items-section");s&&(s.innerHTML=j(),H())};window.addEventListener("ideas:updated",n),window.addEventListener("customitems:updated",o),window.__currentPageCleanup=()=>{window.removeEventListener("ideas:updated",n),window.removeEventListener("customitems:updated",o)}}function Z(){const e=_e();if(e.length===0)return"";const a=e.filter(n=>n.completed).length,t=e.length,i=t?a/t*100:0;return`
    <div class="category-card" id="ideas-checklist-card">
      ...
      <div class="category-items" id="ideas-cl-items">
        ${e.map(n=>`
          <label class="checklist-item ${n.completed?"checked":""}" data-idea-id="${n.id}">
            <input type="checkbox" data-idea-id="${n.id}" ${n.completed?"checked":""} />
            <span class="checklist-item-text">${q(n.text)}</span>
            ${n.location_name?`<span class="priority-badge" style="background:#eff6ff;color:var(--color-primary)">📍 ${q(n.location_name)}</span>`:""}
          </label>
        `).join("")}
  ...
  `}function Y(){var e,a;(e=document.querySelector("#ideas-checklist-card .category-header"))==null||e.addEventListener("click",t=>{var i,n;t.target.tagName!=="INPUT"&&((i=document.getElementById("ideas-cl-items"))==null||i.classList.toggle("hidden"),(n=document.getElementById("ideas-cl-chev"))==null||n.classList.toggle("open"))}),(a=document.getElementById("ideas-cl-items"))==null||a.addEventListener("change",t=>{const i=t.target;if(i.type!=="checkbox")return;const n=i.dataset.ideaId;n&&x(n,{completed:i.checked})})}function j(){const e=w(),a=e.filter(t=>t.checked).length;return`
      ...
      <div class="category-items" id="custom-items-list">
        ${e.length===0?'<div class="custom-items-empty">Nessuna voce personalizzata. Aggiungine una qui sotto.</div>':e.map(t=>`
              <div class="checklist-item custom-item" data-cid="${t.id}">
                ...
                <span class="checklist-item-text ${t.checked?"line-through":""}">${q(t.text)}</span>
                ${t.category!=="Varie"?`<span class="priority-badge" style="background:#f1f5f9;color:#64748b;">${q(t.category)}</span>`:""}
  ...
  `}function H(){var a,t,i;const e=document.getElementById("custom-items-header");e==null||e.addEventListener("click",n=>{var o,s;n.target.closest(".custom-add-form")||n.target.closest("button")||((o=document.getElementById("custom-items-list"))==null||o.classList.toggle("hidden"),(s=document.getElementById("custom-chev"))==null||s.classList.toggle("open"))}),(a=document.getElementById("custom-items-list"))==null||a.addEventListener("change",n=>{const o=n.target;o.classList.contains("custom-item-cb")&&Oe(o.dataset.cid,o.checked)}),(t=document.getElementById("custom-items-list"))==null||t.addEventListener("click",n=>{const o=n.target.closest(".custom-item-del");if(o){Ne(o.dataset.cid);return}const s=n.target.closest(".custom-item-edit");if(s){He(s.dataset.cid);return}if(n.target.closest("#custom-add-btn")){W();return}n.target.closest("#clear-custom-btn")&&confirm("Eliminare tutte le voci personalizzate?")&&Pe()}),(i=document.getElementById("custom-item-text"))==null||i.addEventListener("keydown",n=>{n.key==="Enter"&&(n.preventDefault(),W())})}function W(){var i;const e=document.getElementById("custom-item-text"),a=((i=document.getElementById("custom-item-cat"))==null?void 0:i.value)??"Varie",t=e==null?void 0:e.value.trim();if(!t){e==null||e.focus();return}Te(t,a),e&&(e.value="")}function He(e){const a=w().find(p=>p.id===e);if(!a)return;const t=document.querySelector(`.custom-item[data-cid="${e}"]`);if(!t)return;const i=t.querySelector(".checklist-item-text"),n=t.querySelector(".custom-item-edit"),o=t.querySelector(".custom-item-del"),s=t.querySelector(".priority-badge");if(!i)return;const c=document.createElement("input");c.type="text",c.value=a.text,c.className="custom-edit-input";const l=document.createElement("button");l.textContent="✓",l.className="custom-item-save-edit",l.title="Salva";const d=document.createElement("button");d.textContent="✕",d.className="custom-item-cancel-edit",d.title="Annulla",i.replaceWith(c),s&&(s.style.display="none"),n&&n.replaceWith(l),o&&(o.style.display="none"),l.after(d),c.focus(),c.select();function r(){const p=c.value.trim();if(!p){c.focus();return}De(e,p)}function m(){const p=document.getElementById("custom-items-section");p&&(p.innerHTML=j(),H())}c.addEventListener("keydown",p=>{p.key==="Enter"&&(p.preventDefault(),r()),p.key==="Escape"&&m()}),l.addEventListener("click",r),d.addEventListener("click",m)}function Re(e,a){const t=e.items.length,i=e.items.filter(n=>a[n.id]).length;return`
    <div class="category-card" data-cat="${e.id}">
      ...
      <div class="category-items" id="items-${e.id}">
        ${e.items.map(n=>Fe(n,a[n.id]||!1)).join("")}
      </div>
    </div>
  ...
  `}function Ve(e,a){var t,i,n,o;(t=document.getElementById("checklist-categories"))==null||t.addEventListener("change",s=>{var d;const c=s.target;if(c.type!=="checkbox")return;const l=c.dataset.id;a[l]=c.checked,(d=c.closest(".checklist-item"))==null||d.classList.toggle("checked",c.checked),D(a),Ge(e,a,l),S(e,a)}),document.querySelectorAll(".category-header").forEach(s=>{s.addEventListener("click",c=>{var d,r,m;if(c.target.tagName==="INPUT")return;const l=(d=s.closest(".category-card"))==null?void 0:d.dataset.cat;l&&((r=document.getElementById(`items-${l}`))==null||r.classList.toggle("hidden"),(m=document.getElementById(`chev-${l}`))==null||m.classList.toggle("open"))})}),(i=document.getElementById("check-all"))==null||i.addEventListener("click",()=>{e.forEach(s=>s.items.forEach(c=>{a[c.id]=!0})),D(a),N(e,a),S(e,a)}),(n=document.getElementById("uncheck-all"))==null||n.addEventListener("click",()=>{e.forEach(s=>s.items.forEach(c=>{a[c.id]=!1})),D(a),N(e,a),S(e,a)}),(o=document.getElementById("reset-btn"))==null||o.addEventListener("click",()=>{confirm("Vuoi resettare l'intera checklist?")&&(qe(),e.forEach(s=>s.items.forEach(c=>{a[c.id]=!1})),N(e,a),S(e,a))})}function Ge(e,a,t){const i=e.find(d=>d.items.some(r=>r.id===t));if(!i)return;const n=i.items.length,o=i.items.filter(d=>a[d.id]).length,s=n?o/n*100:0,c=document.getElementById(`count-${i.id}`),l=document.getElementById(`prog-${i.id}`);c&&(c.textContent=`${o}/${n}`),l&&(l.style.width=`${s}%`)}function S(e,a){const t=e.reduce((c,l)=>c+l.items.length,0),i=e.reduce((c,l)=>c+l.items.filter(d=>a[d.id]).length,0),n=t?i/t*100:0,o=document.getElementById("global-fill"),s=document.getElementById("global-text");o&&(o.style.width=`${n}%`),s&&(s.textContent=`${i} / ${t}`)}function N(e,a){e.forEach(t=>{const i=t.items.length,n=t.items.filter(l=>a[l.id]).length,o=i?n/i*100:0,s=document.getElementById(`count-${t.id}`),c=document.getElementById(`prog-${t.id}`);s&&(s.textContent=`${n}/${i}`),c&&(c.style.width=`${o}%`),t.items.forEach(l=>{const d=document.querySelector(`.checklist-item[data-id="${l.id}"]`),r=d==null?void 0:d.querySelector('input[type="checkbox"]');!d||!r||(r.checked=a[l.id]||!1,d.classList.toggle("checked",r.checked))})})}function q(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}let Q=null;async function Je(){const e=document.getElementById("page-content");e.innerHTML=`
    <div class="page-header">
  ...
  `,Q=await L();{Ue(e,Q);return}}function Ue(e,a){document.getElementById("map-outer").innerHTML=`
  ...
  `}let E=null,_="tutte",z=[],f=!1;async function Ke(){const e=document.getElementById("page-content");let a;try{a=await L()}catch(i){e.innerHTML=`<div class="error-state"><div class="error-icon">⚠️</div><h2>Errore</h2><p>${i.message}</p></div>`;return}z=a.days,E=null,_="tutte",e.innerHTML=`
    <div class="page-header">
      <h1>💡 Idee Rapide</h1>
  ...
  `,O(),me(),re(),Qe();const t=()=>{y(),I()};window.addEventListener("ideas:updated",t),window.__currentPageCleanup=()=>window.removeEventListener("ideas:updated",t)}function T(e){var s,c;const a=!!e,t=z.map(l=>`<option value="${l.date}" ${(e==null?void 0:e.day_date)===l.date?"selected":""}>
      Gg. ${l.day} — ${l.location} · ${l.date.slice(5).replace("-","/")}
    </option>`).join(""),i=P.map(l=>`<option value="${l.value}" ${((e==null?void 0:e.stato)??"idea")===l.value?"selected":""}>${l.label}</option>`).join(""),n=ne.map(l=>`<option value="${l.value}" ${((e==null?void 0:e.categoria)??"varia")===l.value?"selected":""}>${l.label}</option>`).join(""),o=oe.map(l=>`<option value="${l.value}" ${((e==null?void 0:e.priorita)??"media")===l.value?"selected":""}>${l.label}</option>`).join("");return`
    <form id="idea-form" class="ideas-form">
      ...
      <div class="ideas-row3">
        <div class="ideas-field-mini">
          ...
          <select id="idea-categoria" class="ideas-select">${n}</select>
        </div>
        <div class="ideas-field-mini">
  ...
  `}function O(){var a,t,i;const e=document.getElementById("ideas-form-wrap");e&&((a=document.getElementById("ideas-toggle-advanced"))==null||a.addEventListener("click",()=>{f=!f;const n=document.getElementById("ideas-advanced"),o=document.getElementById("ideas-toggle-advanced");n==null||n.classList.toggle("hidden",!f),o&&(o.textContent=f?"▲ Meno dettagli":"▼ Più dettagli")}),e.addEventListener("change",n=>{var o;if(n.target.id==="idea-day"){const s=z.find(c=>c.date===n.target.value);s!=null&&s.coordinates&&(document.getElementById("idea-lat").value=s.coordinates.lat,document.getElementById("idea-lng").value=s.coordinates.lng,document.getElementById("idea-coords-hint").textContent=`Auto-riempito da "${s.location}"`)}n.target.id==="idea-add-map"&&((o=document.getElementById("idea-color-row"))==null||o.classList.toggle("hidden",!n.target.checked))}),(t=document.getElementById("idea-form"))==null||t.addEventListener("submit",n=>{n.preventDefault(),Ze()}),(i=document.getElementById("idea-cancel-btn"))==null||i.addEventListener("click",()=>{E=null,document.getElementById("ideas-form-wrap").innerHTML=T(null),O()}))}function Ze(){var s,c,l,d,r,m,p,b,A,u,$,G,J,U;const e=(s=document.getElementById("idea-text"))==null?void 0:s.value.trim();if(!e){(c=document.getElementById("idea-text"))==null||c.focus();return}const a=parseFloat((l=document.getElementById("idea-lat"))==null?void 0:l.value),t=parseFloat((d=document.getElementById("idea-lng"))==null?void 0:d.value),i=!isNaN(a)&&!isNaN(t)?{lat:a,lng:t}:null,n=(r=document.getElementById("idea-add-map"))==null?void 0:r.checked,o={text:e,note:((m=document.getElementById("idea-note"))==null?void 0:m.value.trim())??"",categoria:((p=document.getElementById("idea-categoria"))==null?void 0:p.value)??"varia",stato:((b=document.getElementById("idea-stato"))==null?void 0:b.value)??"idea",priorita:((A=document.getElementById("idea-priorita"))==null?void 0:A.value)??"media",day_date:((u=document.getElementById("idea-day"))==null?void 0:u.value)||null,location_name:(($=document.getElementById("idea-location"))==null?void 0:$.value.trim())||null,link:((G=document.getElementById("idea-link"))==null?void 0:G.value.trim())||"",coordinates:i,add_to_checklist:((J=document.getElementById("idea-add-checklist"))==null?void 0:J.checked)??!1,add_to_map:n&&i!==null,marker_color:((U=document.querySelector('input[name="idea-color"]:checked'))==null?void 0:U.value)??"#f59e0b"};E?(x(E,o),E=null):F(o),f=!1,document.getElementById("ideas-form-wrap").innerHTML=T(null),O(),y(),I()}function de(){const e=g();return[{key:"tutte",label:`Tutte (${e.length})`},{key:"idea",label:`Idea (${e.filter(t=>t.stato==="idea").length})`},{key:"da-verificare",label:`Da verificare (${e.filter(t=>t.stato==="da-verificare").length})`},{key:"prenotare",label:`Prenotare (${e.filter(t=>t.stato==="prenotare").length})`},{key:"approvata",label:`Approvate (${e.filter(t=>t.stato==="approvata").length})`},{key:"scartata",label:`Scartate (${e.filter(t=>t.stato==="scartata").length})`}].map(t=>`
    <button class="idea-filter-btn ${_===t.key?"active":""}" data-f="${t.key}">
      ${t.label}
    </button>
  `).join("")}function re(){var e;(e=document.getElementById("ideas-filters-row"))==null||e.querySelectorAll(".idea-filter-btn").forEach(a=>{a.addEventListener("click",()=>{_=a.dataset.f,I(),y()})})}function I(){const e=document.getElementById("ideas-filters-row");e&&(e.innerHTML=de(),re())}function ue(){let e=g();return _!=="tutte"&&(e=e.filter(a=>a.stato===_)),e.length===0?`<div class="ideas-empty">${_==="tutte"?"Nessuna idea salvata. Aggiungine una qui sopra.":"Nessuna idea per questo filtro."}</div>`:e.map(Ye).join("")}function Ye(e){const a=P.find(o=>o.value===e.stato),t=ne.find(o=>o.value===e.categoria),i=oe.find(o=>o.value===e.priorita),n=e.day_date?z.find(o=>o.date===e.day_date):null;return`
    <div class="idea-card ${e.stato==="scartata"?"idea-card--scartata":""}" data-id="${e.id}">
      <div class="idea-card-main">
        <div class="idea-card-header-row">
          <span class="idea-card-text">${h(e.text)}</span>
          <div class="idea-card-badges-inline">
            ...
            ${i?`<span class="idea-pri-dot" title="${i.label}" style="background:${Xe(e.priorita)}"></span>`:""}
          </div>
        </div>

        ${e.note?`<div class="idea-card-note">${h(e.note)}</div>`:""}

        <div class="idea-card-meta">
          ${n?`<span class="idea-meta-chip">📅 Gg. ${n.day} — ${n.location}</span>`:""}
          ${e.location_name?`<span class="idea-meta-chip">📍 ${h(e.location_name)}</span>`:""}
          ...
          ${e.link?`<a href="${h(e.link)}" target="_blank" rel="noopener" class="idea-meta-chip idea-meta-link">🔗 Link</a>`:""}
        </div>
      </div>
      ...
      <div class="idea-card-actions">
        ...
        <select class="idea-stato-select" data-action="stato" data-id="${e.id}" title="Cambia stato">
          ${P.map(o=>`<option value="${o.value}" ${e.stato===o.value?"selected":""}>${o.label}</option>`).join("")}
        </select>
  ...
  `}function We(e){var n,o,s;const a=g().find(c=>c.id===e);if(!a)return;(n=document.getElementById("day-link-modal"))==null||n.remove();const t=z.map(c=>`<option value="${c.date}" ${a.day_date===c.date?"selected":""}>
  ...
  `,document.body.appendChild(i),(o=document.getElementById("day-link-confirm"))==null||o.addEventListener("click",()=>{var l;const c=((l=document.getElementById("day-link-select"))==null?void 0:l.value)||null;x(e,{day_date:c}),i.remove(),y()}),(s=document.getElementById("day-link-cancel"))==null||s.addEventListener("click",()=>i.remove()),i.addEventListener("click",c=>{c.target===i&&i.remove()})}function me(){const e=document.getElementById("ideas-list");e&&(e.addEventListener("change",a=>{a.target.dataset.action==="stato"&&(x(a.target.dataset.id,{stato:a.target.value}),y(),I())}),e.addEventListener("click",a=>{var o;const t=a.target.closest("[data-action]");if(!t)return;const i=t.dataset.id,n=t.dataset.action;if(n==="delete"){if(!confirm("Eliminare questa idea?"))return;se(i),y(),I()}if(n==="edit"){E=i;const s=g().find(c=>c.id===i);if(!s)return;f=!0,document.getElementById("ideas-form-wrap").innerHTML=T(s),O(),(o=document.getElementById("ideas-form-wrap"))==null||o.scrollIntoView({behavior:"smooth",block:"start"})}if(n==="checklist"){const s=g().find(c=>c.id===i);if(!s)return;x(i,{add_to_checklist:!s.add_to_checklist}),y()}n==="day-link"&&We(i)}))}function y(){const e=document.getElementById("ideas-list");e&&(e.innerHTML=ue(),me())}function Qe(){var e,a;(e=document.getElementById("export-btn"))==null||e.addEventListener("click",()=>{const t=Ie(),i=new Blob([t],{type:"application/json"}),n=URL.createObjectURL(i),o=document.createElement("a");o.href=n,o.download=`idee_viaggio_${new Date().toISOString().slice(0,10)}.json`,o.click(),URL.revokeObjectURL(n)}),(a=document.getElementById("import-file"))==null||a.addEventListener("change",async t=>{var n;const i=(n=t.target.files)==null?void 0:n[0];if(i){try{const o=await i.text(),s=Le(o);alert(`Importazione completata: ${s} nuove idee aggiunte.`),y(),I()}catch(o){alert("Errore importazione: "+o.message)}t.target.value=""}})}function Xe(e){return e==="alta"?"#ef4444":e==="media"?"#f59e0b":"#22c55e"}function h(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const k=[{id:"zlatni-rat",nome:"Zlatni Rat",area:"Brač",categoria:"spiaggia",categoriaLabel:"🏖️ Spiaggia",descrizione:"La spiaggia più fotografata della Croazia: un lungo promontorio di ciottoli bianchi che cambia forma con le correnti, circondato da acque turchesi e pinete.",bambini:!0,bambini_nota:"Ottimo — acque basse, spiaggia servita, pini per l'ombra",impegno:"mezza-giornata",nota_tipo:"molto-consigliato",coords:{lat:43.3246,lng:16.6372}},{id:"vidova-gora",nome:"Vidova Gora",area:"Brač",categoria:"panorama",categoriaLabel:"⛰️ Panorama",descrizione:"Il punto più alto di tutte le isole dalmate (778 m). Vista a 360° su Zlatni Rat, Hvar, il canale e il mare aperto. Si raggiunge in auto su strada asfaltata.",bambini:!0,bambini_nota:"In auto facilissimo; il sentiero a piedi è impegnativo",impegno:"sosta-breve",nota_tipo:"molto-consigliato",coords:{lat:43.3621,lng:16.6588}},{id:"telascica",nome:"Telašćica Nature Park",area:"Area di Zara",categoria:"parco",categoriaLabel:"🌿 Parco naturale",descrizione:"Baia naturale nell'isola di Dugi Otok: scogliere alte 200 m, acqua smeraldo, e il Lago Mir (salato con effetti benefici). Accesso in barca da Zadar o Biograd.",bambini:!0,bambini_nota:"Bello per tutta la famiglia — solo in barca, fare il bagno nel lago è esperienza unica",impegno:"giornata-piena",nota_tipo:"molto-consigliato",coords:{lat:43.9044,lng:15.1533}},{id:"vransko-lake",nome:"Vransko Lake",area:"Area di Zara",categoria:"lago",categoriaLabel:"🦢 Lago / Ornitologia",descrizione:"Il lago più grande della Croazia (30 km²), riserva ornitologica con oltre 250 specie di uccelli. Pista ciclabile panoramica, kayak, ambiente tranquillo.",bambini:!0,bambini_nota:"Adatto — pista ciclabile e percorsi piani ideali con i bambini",impegno:"mezza-giornata",nota_tipo:"opzionale",coords:{lat:43.8856,lng:15.5475}},{id:"paklenica",nome:"Paklenica National Park",area:"Area di Zara",categoria:"parco",categoriaLabel:"🏔️ Canyon / Trekking",descrizione:"Canyon spettacolari ai piedi delle Alpi Dinare. Due gole (Velika e Mala Paklenica) con sentieri per tutti i livelli, pareti d'arrampicata e la grotta Manita Peć.",bambini:!1,bambini_nota:"Sentieri brevi e facili esistono, ma le gole principali sono impegnative per bambini piccoli",impegno:"giornata-piena",nota_tipo:"solo-se-c-e-tempo",coords:{lat:44.3219,lng:15.4722}},{id:"zrmanja",nome:"Canyon Zrmanja",area:"Area di Zara",categoria:"fiume",categoriaLabel:"🌊 Fiume / Rafting",descrizione:"Uno dei fiumi più belli della Dalmazia: acque turchesi carsiche, cascate di travertino e canyon selvaggi. Kayak e rafting disponibili da operatori locali.",bambini:!1,bambini_nota:"Rafting non adatto ai bambini piccoli; belvedere sul canyon accessibile a tutti",impegno:"mezza-giornata",nota_tipo:"opzionale",coords:{lat:44.1867,lng:15.8753}},{id:"brijuni",nome:"Brijuni National Park",area:"Istria",categoria:"parco",categoriaLabel:"🦒 Parco / Safari",descrizione:"Arcipelago di 14 isole con safari park (zebre, elefanti), resti romani e spiagge meravigliose. Accesso solo in traghetto da Fažana (vicino Pola). Era la residenza di Tito.",bambini:!0,bambini_nota:"Fantastico per i bambini — safari su trenino, animali esotici, mare pulito",impegno:"giornata-piena",nota_tipo:"molto-consigliato",coords:{lat:44.9167,lng:13.7636}},{id:"lim-fjord",nome:"Lim Fjord (Limski Kanal)",area:"Istria",categoria:"natura",categoriaLabel:"🌲 Fiordo / Paesaggio",descrizione:"Canale marino di 10 km incastrato tra boschi e vigneti istriani. Famoso per le ostriche allevate in acqua. Belvedere dall'alto accessibile in auto, gita in barca dal basso.",bambini:!0,bambini_nota:"Il belvedere è accessibile a tutti; la barca è ottima per i bambini",impegno:"sosta-breve",nota_tipo:"molto-consigliato",coords:{lat:45.1419,lng:13.6408}},{id:"dobbiaco",nome:"Lago di Dobbiaco",area:"Sesto / Alta Pusteria",categoria:"lago",categoriaLabel:"🏔️ Lago alpino",descrizione:"Lago alpino in fondo alla Val Pusteria, a pochi km da Sesto. Raggiungibile a piedi o in bici sulla famosa pista ciclabile. Acque verdissime, Dolomiti come sfondo.",bambini:!0,bambini_nota:"Perfetto — pista ciclabile pianeggiante, riva accessibile, area pic-nic",impegno:"sosta-breve",nota_tipo:"molto-consigliato",coords:{lat:46.7358,lng:12.2214}},{id:"braies",nome:"Lago di Braies",area:"Sesto / Alta Pusteria",categoria:"lago",categoriaLabel:"🏔️ Lago alpino",descrizione:'Il "lago delle fiabe" delle Dolomiti: acque verde-smeraldo ai piedi delle Dolomiti di Sesto. Barche a remi, sentiero attorno al lago (3 km). Molto frequentato in agosto — meglio la mattina presto.',bambini:!0,bambini_nota:"Adatto — sentiero pianeggiante attorno al lago, barche a noleggio",impegno:"mezza-giornata",nota_tipo:"molto-consigliato",coords:{lat:46.6948,lng:12.0853}}],et={"sosta-breve":{label:"Sosta breve",icon:"⚡",cls:"impegno-breve"},"mezza-giornata":{label:"Mezza giornata",icon:"🕐",cls:"impegno-mezza"},"giornata-piena":{label:"Giornata piena",icon:"📅",cls:"impegno-piena"}},tt={"molto-consigliato":{label:"⭐ Molto consigliato",cls:"nota-top"},opzionale:{label:"✔ Opzionale",cls:"nota-ok"},"solo-se-c-e-tempo":{label:"⏱ Solo se c'è tempo",cls:"nota-ifpos"}};async function at(){var i;const e=document.getElementById("page-content"),a=[...new Set(k.map(n=>n.area))];let t="tutte";e.innerHTML=`
    ...
    <div class="natura-filters" id="natura-filters">
      ...
      ${a.map(n=>`
        <button class="natura-filter-btn" data-area="${n}">
          ${n} (${k.filter(o=>o.area===n).length})
        </button>
      `).join("")}
    </div>
  ...
  `}function ee(){document.querySelectorAll(".natura-add-idea-btn").forEach(e=>{e.addEventListener("click",()=>{const a=e.dataset.nome,t=e.dataset.area,i=parseFloat(e.dataset.lat),n=parseFloat(e.dataset.lng);F({text:a,location_name:t,categoria:"escursione",stato:"da-verificare",add_to_map:!0,coordinates:{lat:i,lng:n},marker_color:"#10b981"}),e.textContent="✅ Aggiunta!",e.disabled=!0,setTimeout(()=>{e.textContent="💡 Aggiungi alle Idee",e.disabled=!1},2e3)})})}function te(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const ae={"#dashboard":fe,"#itinerary":we,"#hotels":ze,"#checklist":je,"#map":Je,"#ideas":Ke,"#natura":at};async function pe(){typeof window.__currentPageCleanup=="function"&&(window.__currentPageCleanup(),window.__currentPageCleanup=null);const e=window.location.hash||"#dashboard",a=ae[e]??ae["#dashboard"],t=document.getElementById("page-content");t.innerHTML=`
    <div class="loading">
