(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))i(o);new MutationObserver(o=>{for(const n of o)if(n.type==="childList")for(const s of n.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function t(o){const n={};return o.integrity&&(n.integrity=o.integrity),o.referrerPolicy&&(n.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?n.credentials="include":o.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function i(o){if(o.ep)return;o.ep=!0;const n=t(o);fetch(o.href,n)}})();function Ee(){const e=document.getElementById("nav-toggle"),a=document.getElementById("nav-links");e==null||e.addEventListener("click",()=>{a==null||a.classList.toggle("open")}),a==null||a.querySelectorAll(".nav-link").forEach(i=>{i.addEventListener("click",()=>{a.classList.remove("open")})});function t(){const i=window.location.hash||"#dashboard";document.querySelectorAll(".nav-link").forEach(o=>{o.classList.toggle("active",o.getAttribute("href")===i)})}window.addEventListener("hashchange",t),t()}let q=null;async function L(){if(q)return q;const t=await fetch("/viaggio-croazia/data/trip.json");if(!t.ok)throw new Error(`Impossibile caricare trip.json (${t.status})`);return q=await t.json(),q}function f(e){return e?new Date(e+"T00:00:00").toLocaleDateString("it-IT",{day:"numeric",month:"short",year:"numeric"}):""}function _e(e){return e?new Date(e+"T00:00:00").toLocaleDateString("it-IT",{weekday:"short"}).toUpperCase():""}function ee(e){const a=new Date;a.setHours(0,0,0,0);const t=new Date(e+"T00:00:00");return Math.round((t-a)/(1e3*60*60*24))}function Ie(e){return"★".repeat(e)+"☆".repeat(5-e)}async function we(){const e=document.getElementById("page-content");let a;try{a=await L()}catch(u){e.innerHTML=xe(u.message);return}const{meta:t,days:i,hotels:o}=a,n=ee(t.start_date),s=ee(t.end_date),c=new Date().toISOString().slice(0,10),l=i.find(u=>u.date===c),d=i.find(u=>u.date>c),r=l||d,m=[],p=new Set;for(const u of i)p.has(u.location)||(p.add(u.location),m.push({location:u.location,day:u.day,date:u.date}));const b=o.filter(u=>u.recommended),M=b.reduce((u,k)=>u+k.nights,0);b.filter(u=>u.price_per_night>0).reduce((u,k)=>u+k.nights*k.price_per_night,0),e.innerHTML=`
  ...
  `}const re="viaggio_croazia_ideas_v1",ue=[{value:"alloggio",label:"🏨 Alloggio"},{value:"ristorante",label:"🍽️ Ristorante"},{value:"esperienza",label:"🎯 Esperienza"},{value:"spiaggia",label:"🏖️ Spiaggia"},{value:"escursione",label:"🥾 Escursione"},{value:"cantina",label:"🍷 Cantina / Cibo"},{value:"shopping",label:"🛍️ Shopping"},{value:"varia",label:"💡 Varia"}],F=[{value:"idea",label:"Idea",color:"#6366f1"},{value:"da-verificare",label:"Da verificare",color:"#f59e0b"},{value:"prenotare",label:"Prenotare",color:"#ef4444"},{value:"approvata",label:"Approvata",color:"#10b981"},{value:"scartata",label:"Scartata",color:"#94a3b8"}],me=[{value:"alta",label:"🔴 Alta"},{value:"media",label:"🟡 Media"},{value:"bassa",label:"🟢 Bassa"}],Se=[{value:"#f59e0b",label:"Giallo"},{value:"#ef4444",label:"Rosso"},{value:"#8b5cf6",label:"Viola"},{value:"#10b981",label:"Verde"},{value:"#f97316",label:"Arancio"}];function Be(){return"idea_"+Date.now().toString(36)+"_"+Math.random().toString(36).slice(2,5)}function J(e){return{categoria:"varia",link:"",priorita:"media",stato:"idea",note:e.note??"",location_name:e.location_name??null,...e}}function g(){try{const e=localStorage.getItem(re);return e?JSON.parse(e).map(J):[]}catch{return[]}}function D(e){try{localStorage.setItem(re,JSON.stringify(e)),window.dispatchEvent(new CustomEvent("ideas:updated",{detail:{ideas:e}}))}catch(a){console.warn("[ideas] localStorage non disponibile:",a)}}function K(e){const a=g(),t=J({id:Be(),text:"",note:"",categoria:"varia",link:"",priorita:"media",stato:"idea",created_at:new Date().toISOString(),day_date:null,location_name:null,coordinates:null,add_to_checklist:!1,add_to_map:!1,marker_color:"#f59e0b",completed:!1,...e});return a.unshift(t),D(a),t}function B(e,a){const t=g(),i=t.findIndex(o=>o.id===e);return i===-1?null:(t[i]={...t[i],...a},D(t),t[i])}function pe(e){D(g().filter(a=>a.id!==e))}function ze(e){return g().filter(a=>a.day_date===e)}function Ce(){return g().filter(e=>e.add_to_checklist)}function P(){return g().filter(e=>{var a;return e.add_to_map&&((a=e.coordinates)==null?void 0:a.lat)!=null})}function Ae(){return JSON.stringify(g(),null,2)}function Me(e){const a=JSON.parse(e);if(!Array.isArray(a))throw new Error("JSON non valido: deve essere un array");const t=g(),i=new Set(t.map(s=>s.id));let o=0;const n=[...t];return a.forEach(s=>{i.has(s.id)||(n.push(J(s)),o++)}),D(n),o}async function qe(){var c,l;const e=document.getElementById("page-content");let a;try{a=await L()}catch(d){e.innerHTML=`<div class="error-state"><div class="error-icon">⚠️</div><h2>Errore</h2><p>${d.message}</p></div>`;return}const{days:t,hotels:i,meta:o}=a,n=Object.fromEntries(i.map(d=>[d.id,d]));e.innerHTML=`
    ...
    <div class="timeline-item">
      ...
      <div class="timeline-left">
        ...
        <div class="timeline-date-label">
          ${_e(e.date)}<br>${e.date.slice(5).replace("-","/")}
        </div>
      </div>
  ...
  `}function ge(e){const a=ze(e);return a.length===0?"":a.map(t=>`
    <div class="day-idea-pill ${t.completed?"day-idea-pill--done":""}" data-id="${t.id}">
      <span class="day-idea-text">${De(t.text)}</span>
  ...
  `).join("")}function Oe(e){const a=document.querySelector(".timeline");a&&(a.addEventListener("click",t=>{var s,c;const i=t.target.closest(".day-add-idea-btn");if(i){const l=i.dataset.date,d=document.getElementById(`quick-form-${l}`);if(!d)return;const r=d.classList.contains("hidden");document.querySelectorAll(".day-quick-form").forEach(m=>m.classList.add("hidden")),r&&(d.classList.remove("hidden"),(s=d.querySelector(".quick-idea-text"))==null||s.focus())}const o=t.target.closest(".quick-cancel");o&&((c=o.closest(".day-quick-form"))==null||c.classList.add("hidden"));const n=t.target.closest(".day-idea-del");if(n){const l=n.dataset.id;confirm("Eliminare questa idea?")&&pe(l)}}),a.addEventListener("submit",t=>{var d,r,m,p;const i=t.target.closest(".day-quick-form");if(!i)return;t.preventDefault();const o=(d=i.querySelector(".quick-idea-text"))==null?void 0:d.value.trim();if(!o){(r=i.querySelector(".quick-idea-text"))==null||r.focus();return}const n=i.dataset.date,s=e.find(b=>b.date===n),c=(m=i.querySelector(".quick-map"))==null?void 0:m.checked,l=(p=i.querySelector(".quick-checklist"))==null?void 0:p.checked;K({text:o,day_date:n,location_name:(s==null?void 0:s.location)??null,coordinates:c&&(s!=null&&s.coordinates)?s.coordinates:null,add_to_checklist:!!l,add_to_map:!!(c&&(s!=null&&s.coordinates))}),i.querySelector(".quick-idea-text").value="",i.querySelector(".quick-map").checked=!1,i.querySelector(".quick-checklist").checked=!1,i.classList.add("hidden")}))}function De(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}async function Pe(){const e=document.getElementById("page-content");let a;try{a=await L()}catch(l){e.innerHTML=`<div class="error-state"><div class="error-icon">⚠️</div><h2>Errore</h2><p>${l.message}</p></div>`;return}const{hotels:t}=a,i={},o=[];for(const l of t){const d=l.location_group||l.location;i[d]||(i[d]=[],o.push(d)),i[d].push(l)}const n=t.filter(l=>l.recommended&&l.price_per_night>0),s=n.reduce((l,d)=>l+d.nights,0),c=n.reduce((l,d)=>l+d.nights*d.price_per_night,0);e.innerHTML=`
    <div class="page-header">
      <h1>🏨 Hotel &amp; Alloggi</h1>
    ...
    `}).join("")}
  `}function Ne(e){const a=e.nights,t=a*e.price_per_night,o=e.booking_ref&&e.booking_ref.length>0?'<span class="hotel-status-badge badge-prenotato">✅ Prenotato</span>':e.status==="da_confermare"?'<span class="hotel-status-badge badge-confermare">⚠️ Da confermare</span>':'<span class="hotel-status-badge badge-da-prenotare">📋 Da prenotare</span>',n=e.recommended?'<span class="hotel-recommended-badge">⭐ Consigliato</span>':'<span class="hotel-alt-badge">Alternativa</span>';return`
      ...
      <div class="hotel-card-body">
        ...
        <div class="hotel-meta">
          <div class="hotel-rating" title="${e.rating} stelle">
            ${e.rating>0?Ie(e.rating):'<span style="color:var(--color-text-muted);font-size:0.78rem;">Categoria n.d.</span>'}
          </div>
  ...
  `}const Z="viaggio_croazia_checklist_v1",ve="viaggio_croazia_custom_items_v1";function He(){try{const e=localStorage.getItem(Z);return e?JSON.parse(e):{}}catch{return{}}}function j(e){try{localStorage.setItem(Z,JSON.stringify(e))}catch(a){console.warn("[storage] Impossibile salvare checklist:",a)}}function je(){try{localStorage.removeItem(Z)}catch{}}function Re(){return"ci_"+Date.now().toString(36)+"_"+Math.random().toString(36).slice(2,5)}function x(){try{const e=localStorage.getItem(ve);return e?JSON.parse(e):[]}catch{return[]}}function C(e){try{localStorage.setItem(ve,JSON.stringify(e)),window.dispatchEvent(new CustomEvent("customitems:updated"))}catch(a){console.warn("[storage] Impossibile salvare custom items:",a)}}function Fe(e,a="Varie"){const t=x(),i={id:Re(),text:e.trim(),category:a,checked:!1,created_at:new Date().toISOString()};return t.unshift(i),C(t),i}function Ve(e,a){C(x().map(t=>t.id===e?{...t,checked:a}:t))}function Ge(e,a){const t=a.trim();t&&C(x().map(i=>i.id===e?{...i,text:t}:i))}function Ue(e){C(x().filter(a=>a.id!==e))}function We(){C([])}async function Je(){const e=document.getElementById("page-content");let a;try{a=await L()}catch(s){e.innerHTML=`<div class="error-state"><div class="error-icon">⚠️</div><h2>Errore</h2><p>${s.message}</p></div>`;return}const{categories:t}=a.checklist,i=He();e.innerHTML=`
    <div class="page-header">
  ...
  `,S(t,i),Qe(t,i),ae(),G();const o=()=>{const s=document.getElementById("checklist-ideas-section");s&&(s.innerHTML=te(),ae())},n=()=>{const s=document.getElementById("custom-items-section");s&&(s.innerHTML=V(),G())};window.addEventListener("ideas:updated",o),window.addEventListener("customitems:updated",n),window.__currentPageCleanup=()=>{window.removeEventListener("ideas:updated",o),window.removeEventListener("customitems:updated",n)}}function te(){const e=Ce();if(e.length===0)return"";const a=e.filter(o=>o.completed).length,t=e.length,i=t?a/t*100:0;return`
  ...
  `}function G(){var a,t,i;const e=document.getElementById("custom-items-header");e==null||e.addEventListener("click",o=>{var n,s;o.target.closest(".custom-add-form")||o.target.closest("button")||((n=document.getElementById("custom-items-list"))==null||n.classList.toggle("hidden"),(s=document.getElementById("custom-chev"))==null||s.classList.toggle("open"))}),(a=document.getElementById("custom-items-list"))==null||a.addEventListener("change",o=>{const n=o.target;n.classList.contains("custom-item-cb")&&Ve(n.dataset.cid,n.checked)}),(t=document.getElementById("custom-items-list"))==null||t.addEventListener("click",o=>{const n=o.target.closest(".custom-item-del");if(n){Ue(n.dataset.cid);return}const s=o.target.closest(".custom-item-edit");if(s){Ke(s.dataset.cid);return}if(o.target.closest("#custom-add-btn")){ie();return}o.target.closest("#clear-custom-btn")&&confirm("Eliminare tutte le voci personalizzate?")&&We()}),(i=document.getElementById("custom-item-text"))==null||i.addEventListener("keydown",o=>{o.key==="Enter"&&(o.preventDefault(),ie())})}function ie(){var i;const e=document.getElementById("custom-item-text"),a=((i=document.getElementById("custom-item-cat"))==null?void 0:i.value)??"Varie",t=e==null?void 0:e.value.trim();if(!t){e==null||e.focus();return}Fe(t,a),e&&(e.value="")}function Ke(e){const a=x().find(p=>p.id===e);if(!a)return;const t=document.querySelector(`.custom-item[data-cid="${e}"]`);if(!t)return;const i=t.querySelector(".checklist-item-text"),o=t.querySelector(".custom-item-edit"),n=t.querySelector(".custom-item-del"),s=t.querySelector(".priority-badge");if(!i)return;const c=document.createElement("input");c.type="text",c.value=a.text,c.className="custom-edit-input";const l=document.createElement("button");l.textContent="✓",l.className="custom-item-save-edit",l.title="Salva";const d=document.createElement("button");d.textContent="✕",d.className="custom-item-cancel-edit",d.title="Annulla",i.replaceWith(c),s&&(s.style.display="none"),o&&o.replaceWith(l),n&&(n.style.display="none"),l.after(d),c.focus(),c.select();function r(){const p=c.value.trim();if(!p){c.focus();return}Ge(e,p)}function m(){const p=document.getElementById("custom-items-section");p&&(p.innerHTML=V(),G())}c.addEventListener("keydown",p=>{p.key==="Enter"&&(p.preventDefault(),r()),p.key==="Escape"&&m()}),l.addEventListener("click",r),d.addEventListener("click",m)}function Ze(e,a){const t=e.items.length,i=e.items.filter(o=>a[o.id]).length;return`
    <div class="category-card" data-cat="${e.id}">
      ...
      <div class="category-items" id="items-${e.id}">
        ${e.items.map(o=>Ye(o,a[o.id]||!1)).join("")}
      </div>
    </div>
  `}function Ye(e,a){return`
  ...
  `,oe=await L(),await at(et),it(oe),ct(),ne();const a=()=>{if(!v)return;z.forEach(i=>i.setMap(null)),z=[],fe(),ne();const t=document.querySelector('.map-btn[data-layer="ideas"]');if(t){const i=P().length;t.textContent=`💡 Idee${i>0?` (${i})`:""}`}};window.addEventListener("ideas:updated",a),window.__currentPageCleanup=()=>window.removeEventListener("ideas:updated",a)}function at(e){return new Promise((a,t)=>{var n;if((n=window.google)!=null&&n.maps){a();return}const i="__gmapsReady_"+Date.now();window[i]=()=>{delete window[i],a()};const o=document.createElement("script");o.src=`https://maps.googleapis.com/maps/api/js?key=${e}&callback=${i}&loading=async`,o.async=!0,o.onerror=()=>t(new Error("Impossibile caricare Google Maps")),document.head.appendChild(o)})}function it(e){const a=document.getElementById("google-map");a&&(v=new google.maps.Map(a,{center:{lat:44,lng:15.5},zoom:7,mapTypeId:"roadmap",styles:[{featureType:"water",elementType:"geometry",stylers:[{color:"#a2daf5"}]},{featureType:"landscape",stylers:[{color:"#f5f5f0"}]}]}),ot(e),nt(e),st(e),fe(),lt(e))}function ot(e){W=[];const a=new Set;e.days.filter(t=>t.coordinates).forEach(t=>{const i=`${t.coordinates.lat},${t.coordinates.lng}`;if(a.has(i))return;a.add(i);const o=new google.maps.Marker({position:t.coordinates,map:v,title:t.location,label:{text:String(t.day),color:"#fff",fontWeight:"bold",fontSize:"11px"},icon:{path:google.maps.SymbolPath.CIRCLE,scale:16,fillColor:"#1e40af",fillOpacity:1,strokeColor:"#ffffff",strokeWeight:2},zIndex:10}),n=new google.maps.InfoWindow({content:`
        <div style="font-family:system-ui,sans-serif;max-width:220px;">
          <strong style="color:#1e40af;">Giorno ${t.day} — ${t.location}</strong>
          <div style="font-size:12px;color:#64748b;margin-top:2px;">${t.date}</div>
          <div style="margin-top:6px;font-size:13px;">${t.title}</div>
        </div>`});o.addListener("click",()=>n.open(v,o)),W.push(o)})}function nt(e){U=[],e.hotels.filter(a=>a.recommended).forEach(a=>{const t=new google.maps.Marker({position:a.coordinates,map:v,title:a.name,icon:{path:google.maps.SymbolPath.BACKWARD_CLOSED_ARROW,scale:7,fillColor:"#059669",fillOpacity:1,strokeColor:"#ffffff",strokeWeight:1.5},zIndex:5}),i=new google.maps.InfoWindow({content:`
        ...
        </div>`});a.addListener("click",()=>t.open(v,a)),z.push(a)})}function st(e){T=new google.maps.Polyline({path:e.days.filter(a=>a.coordinates).map(a=>a.coordinates),geodesic:!0,strokeColor:"#3b82f6",strokeOpacity:.7,strokeWeight:3,icons:[{icon:{path:google.maps.SymbolPath.FORWARD_CLOSED_ARROW,scale:3},repeat:"120px"}],map:v})}function lt(e){const a=new google.maps.LatLngBounds;e.days.filter(t=>t.coordinates).forEach(t=>a.extend(t.coordinates)),e.hotels.filter(t=>t.recommended).forEach(t=>a.extend(t.coordinates)),v.fitBounds(a,60)}function ct(){var t;const e=P().length,a=document.querySelector('.map-btn[data-layer="ideas"]');a&&e>0&&(a.textContent=`💡 Idee (${e})`),(t=document.getElementById("map-controls"))==null||t.addEventListener("click",i=>{const o=i.target.closest(".map-btn");if(!o)return;document.querySelectorAll(".map-btn").forEach(s=>s.classList.remove("active")),o.classList.add("active");const n=o.dataset.layer;W.forEach(s=>s.setVisible(n==="all"||n==="route")),U.forEach(s=>s.setVisible(n==="all"||n==="hotels")),z.forEach(s=>s.setVisible(n==="all"||n==="ideas")),T==null||T.setVisible(n==="all"||n==="route")})}function ne(){const e=document.getElementById("map-legend");if(!e)return;const a=P().length;e.innerHTML=`
  ...
  `}let _=null,I="tutte",A=[],h=!1;async function dt(){const e=document.getElementById("page-content");let a;try{a=await L()}catch(i){e.innerHTML=`<div class="error-state"><div class="error-icon">⚠️</div><h2>Errore</h2><p>${i.message}</p></div>`;return}A=a.days,_=null,I="tutte",e.innerHTML=`
    <div class="page-header">
      <h1>💡 Idee Rapide</h1>
  ...
  `,H(),$e(),he(),pt();const t=()=>{$(),w()};window.addEventListener("ideas:updated",t),window.__currentPageCleanup=()=>window.removeEventListener("ideas:updated",t)}function N(e){var s,c;const a=!!e,t=A.map(l=>`<option value="${l.date}" ${(e==null?void 0:e.day_date)===l.date?"selected":""}>
      Gg. ${l.day} — ${l.location} · ${l.date.slice(5).replace("-","/")}
    </option>`).join(""),i=F.map(l=>`<option value="${l.value}" ${((e==null?void 0:e.stato)??"idea")===l.value?"selected":""}>${l.label}</option>`).join(""),o=ue.map(l=>`<option value="${l.value}" ${((e==null?void 0:e.categoria)??"varia")===l.value?"selected":""}>${l.label}</option>`).join(""),n=me.map(l=>`<option value="${l.value}" ${((e==null?void 0:e.priorita)??"media")===l.value?"selected":""}>${l.label}</option>`).join("");return`
  ...
  `}function H(){var a,t,i;const e=document.getElementById("ideas-form-wrap");e&&((a=document.getElementById("ideas-toggle-advanced"))==null||a.addEventListener("click",()=>{h=!h;const o=document.getElementById("ideas-advanced"),n=document.getElementById("ideas-toggle-advanced");o==null||o.classList.toggle("hidden",!h),n&&(n.textContent=h?"▲ Meno dettagli":"▼ Più dettagli")}),e.addEventListener("change",o=>{var n;if(o.target.id==="idea-day"){const s=A.find(c=>c.date===o.target.value);s!=null&&s.coordinates&&(document.getElementById("idea-lat").value=s.coordinates.lat,document.getElementById("idea-lng").value=s.coordinates.lng,document.getElementById("idea-coords-hint").textContent=`Auto-riempito da "${s.location}"`)}o.target.id==="idea-add-map"&&((n=document.getElementById("idea-color-row"))==null||n.classList.toggle("hidden",!o.target.checked))}),(t=document.getElementById("idea-form"))==null||t.addEventListener("submit",o=>{o.preventDefault(),rt()}),(i=document.getElementById("idea-cancel-btn"))==null||i.addEventListener("click",()=>{_=null,document.getElementById("ideas-form-wrap").innerHTML=N(null),H()}))}function rt(){var s,c,l,d,r,m,p,b,M,u,k,Y,Q,X;const e=(s=document.getElementById("idea-text"))==null?void 0:s.value.trim();if(!e){(c=document.getElementById("idea-text"))==null||c.focus();return}const a=parseFloat((l=document.getElementById("idea-lat"))==null?void 0:l.value),t=parseFloat((d=document.getElementById("idea-lng"))==null?void 0:d.value),i=!isNaN(a)&&!isNaN(t)?{lat:a,lng:t}:null,o=(r=document.getElementById("idea-add-map"))==null?void 0:r.checked,n={text:e,note:((m=document.getElementById("idea-note"))==null?void 0:m.value.trim())??"",categoria:((p=document.getElementById("idea-categoria"))==null?void 0:p.value)??"varia",stato:((b=document.getElementById("idea-stato"))==null?void 0:b.value)??"idea",priorita:((M=document.getElementById("idea-priorita"))==null?void 0:M.value)??"media",day_date:((u=document.getElementById("idea-day"))==null?void 0:u.value)||null,location_name:((k=document.getElementById("idea-location"))==null?void 0:k.value.trim())||null,link:((Y=document.getElementById("idea-link"))==null?void 0:Y.value.trim())||"",coordinates:i,add_to_checklist:((Q=document.getElementById("idea-add-checklist"))==null?void 0:Q.checked)??!1,add_to_map:o&&i!==null,marker_color:((X=document.querySelector('input[name="idea-color"]:checked'))==null?void 0:X.value)??"#f59e0b"};_?(B(_,n),_=null):K(n),h=!1,document.getElementById("ideas-form-wrap").innerHTML=N(null),H(),$(),w()}function be(){const e=g();return[{key:"tutte",label:`Tutte (${e.length})`},{key:"idea",label:`Idea (${e.filter(t=>t.stato==="idea").length})`},{key:"da-verificare",label:`Da verificare (${e.filter(t=>t.stato==="da-verificare").length})`},{key:"prenotare",label:`Prenotare (${e.filter(t=>t.stato==="prenotare").length})`},{key:"approvata",label:`Approvate (${e.filter(t=>t.stato==="approvata").length})`},{key:"scartata",label:`Scartate (${e.filter(t=>t.stato==="scartata").length})`}].map(t=>`
    <button class="idea-filter-btn ${I===t.key?"active":""}" data-f="${t.key}">
      ${t.label}
    </button>
  `).join("")}function he(){var e;(e=document.getElementById("ideas-filters-row"))==null||e.querySelectorAll(".idea-filter-btn").forEach(a=>{a.addEventListener("click",()=>{I=a.dataset.f,w(),$()})})}function w(){const e=document.getElementById("ideas-filters-row");e&&(e.innerHTML=be(),he())}function ye(){let e=g();return I!=="tutte"&&(e=e.filter(a=>a.stato===I)),e.length===0?`<div class="ideas-empty">${I==="tutte"?"Nessuna idea salvata. Aggiungine una qui sopra.":"Nessuna idea per questo filtro."}</div>`:e.map(ut).join("")}function ut(e){const a=F.find(n=>n.value===e.stato),t=ue.find(n=>n.value===e.categoria),i=me.find(n=>n.value===e.priorita),o=e.day_date?A.find(n=>n.date===e.day_date):null;return`
      ...
      <div class="idea-card-main">
        <div class="idea-card-header-row">
          ...
          <div class="idea-card-badges-inline">
            ...
            ${i?`<span class="idea-pri-dot" title="${i.label}" style="background:${gt(e.priorita)}"></span>`:""}
          </div>
        </div>
  ...
  `}function mt(e){var o,n,s;const a=g().find(c=>c.id===e);if(!a)return;(o=document.getElementById("day-link-modal"))==null||o.remove();const t=A.map(c=>`<option value="${c.date}" ${a.day_date===c.date?"selected":""}>
  ...
  `}function le(){document.querySelectorAll(".natura-add-idea-btn").forEach(e=>{e.addEventListener("click",()=>{const a=e.dataset.nome,t=e.dataset.area,i=parseFloat(e.dataset.lat),o=parseFloat(e.dataset.lng);K({text:a,location_name:t,categoria:"escursione",stato:"da-verificare",add_to_map:!0,coordinates:{lat:i,lng:o},marker_color:"#10b981"}),e.textContent="✅ Aggiunta!",e.disabled=!0,setTimeout(()=>{e.textContent="💡 Aggiungi alle Idee",e.disabled=!1},2e3)})})}function ce(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const de={"#dashboard":we,"#itinerary":qe,"#hotels":Pe,"#checklist":Je,"#map":tt,"#ideas":dt,"#natura":bt};async function ke(){typeof window.__currentPageCleanup=="function"&&(window.__currentPageCleanup(),window.__currentPageCleanup=null);const e=window.location.hash||"#dashboard",a=de[e]??de["#dashboard"],t=document.getElementById("page-content");t.innerHTML=`
    <div class="loading">
  ...
  `;try{await a()}catch(i){console.error("[router]",i),t.innerHTML=`
    ...
    `}}Ee();window.addEventListener("hashchange",ke);ke();
